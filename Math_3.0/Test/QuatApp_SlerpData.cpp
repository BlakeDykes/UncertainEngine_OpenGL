...
#if QuatApp_SlerpData

float factorD = 0.6725f;
...
#define numQuatD (sizeof(QuatSourceD)/sizeof(QuatSourceD[0]))
...
TEST(QuatApp_SlerpData, TestConfig::ALL)
{
#if QuatApp_SlerpData
	...
	i = 0;
	QuatApp::Slerp(out1, QuatSourceD[i], QuatTargetD[i], factorD);

	CHECK(eq_float(out1[x], QuatResultD[i][x]));
	CHECK(eq_float(out1[y], QuatResultD[i][y]));
	CHECK(eq_float(out1[z], QuatResultD[i][z]));
	CHECK(eq_float(out1[w], QuatResultD[i][w]));
	...
	i = 22;
	QuatApp::Slerp(out1, QuatSourceD[i], QuatTargetD[i], 1.0f);

	CHECK(eq_float(out1[x], -QuatTargetD[i][x]));
	CHECK(eq_float(out1[y], -QuatTargetD[i][y]));
	CHECK(eq_float(out1[z], -QuatTargetD[i][z]));
	CHECK(eq_float(out1[w], -QuatTargetD[i][w]));

	i = 22;
	QuatApp::Slerp(out1, QuatSourceD[i], QuatTargetD[i], 0.0f);

	CHECK(eq_float(out1[x], QuatSourceD[i][x]));
	CHECK(eq_float(out1[y], QuatSourceD[i][y]));
	CHECK(eq_float(out1[z], QuatSourceD[i][z]));
	CHECK(eq_float(out1[w], QuatSourceD[i][w]));
