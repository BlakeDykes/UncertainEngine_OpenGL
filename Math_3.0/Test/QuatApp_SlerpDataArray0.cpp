...
#define numQuatB (sizeof(QuatSourceB)/sizeof(QuatSourceB[0]))
...
TEST(QuatApp_SlerpDataArray0, TestConfig::ALL)
{
#if QuatApp_SlerpDataArray0
	...
	// Slerp Data Test	
	Quat out2[numQuatB];
	QuatApp::SlerpArray(out2, QuatSourceB, QuatTargetB, 0.0f, numQuatB);

	i = 0;
	CHECK(eq_float(out2[i][x], QuatSourceB[i][x]));
	CHECK(eq_float(out2[i][y], QuatSourceB[i][y]));
	CHECK(eq_float(out2[i][z], QuatSourceB[i][z]));
	CHECK(eq_float(out2[i][w], QuatSourceB[i][w]));
