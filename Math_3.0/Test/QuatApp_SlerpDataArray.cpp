...
TEST(QuatApp_SlerpDataArray, TestConfig::ALL)
{
#if QuatApp_SlerpDataArray
	...
	// Slerp Data Test	
	Quat out2[numQuatA];
	QuatApp::SlerpArray(out2, QuatSourceA, QuatTargetA, factorA, numQuatA);

	i = 0;
	CHECK(eq_float(out2[i][x], QuatResultA[i][x]));
	CHECK(eq_float(out2[i][y], QuatResultA[i][y]));
	CHECK(eq_float(out2[i][z], QuatResultA[i][z]));
	CHECK(eq_float(out2[i][w], QuatResultA[i][w]));
